bl_info = {
    "name": "IRIMeS — Facial Biometric Landmarks",
    "author": "Daniele Bursich (Ph.D.) — University of Verona",
    "version": (6, 0, 0),
    "blender": (3, 0, 0),
    "location": "View3D > Sidebar > Biometry",
    "description": "27 Farkas (1994) landmarks for anthropometric analysis of 3D facial scans. Part of the RESP project (ERC Cog - G.A.: 101002763).",
    "doc_url": "https://irimes.dcuci.univr.it/",
    "tracker_url": "https://irimes.dcuci.univr.it/",
    "category": "Mesh",
    "support": "COMMUNITY",
}
...
def _col_name(mn): return f"FacialLandmarks_{mn}"
def _active_mesh():
    try: return bpy.context.scene.facial_biom.target_mesh
...
def empty_name(lid, mn=None): mn=mn or _active_mesh(); return f"LM_{mn}_{lid}"
def get_lm_pos(lid, mn=None): n=empty_name(lid,mn); return tuple(bpy.data.objects[n].location) if n in bpy.data.objects else None
def lm_placed(lid, mn=None): return empty_name(lid,mn) in bpy.data.objects
def get_active_lid(context):
    o=getattr(context,"active_object",None); return o.get("facial_landmark_id") if o else None
def get_or_create_col(mn=None):
    mn=mn or _active_mesh(); cname=_col_name(mn)
    if cname not in bpy.data.collections:
        c=bpy.data.collections.new(cname); bpy.context.scene.collection.children.link(c)
    return bpy.data.collections[cname]
def create_lm_empty(lid, pos, group, mn=None):
    mn=mn or _active_mesh(); col=get_or_create_col(mn); name=empty_name(lid,mn)
    ...
    col.objects.link(e); e["facial_landmark_id"]=lid; e["facial_mesh_name"]=mn; return e
def get_all_sessions():
    return [c.name[len("FacialLandmarks_"):] for c in bpy.data.collections if c.name.startswith("FacialLandmarks_")]
def delete_session(mn):
    cname=_col_name(mn)
    ...
    def modal(self,c,e):
        ...
        if (e.type=='LEFTMOUSE' and e.value=='PRESS'
            ...
            if hit:
                ...
                col.objects.link(em); em["facial_mesh_name"]=p.target_mesh; _wizard_placed=True; _tag()
...
class FBIO_OT_DeleteAll(Operator):
    ...
    def execute(self,c):
        mn=c.scene.facial_biom.target_mesh
        for d in LANDMARK_DEFS:
            n=empty_name(d[0],mn)
            if n in bpy.data.objects: bpy.data.objects.remove(bpy.data.objects[n],do_unlink=True)
        _tag(); return {'FINISHED'}
...
class FBIO_OT_SnapToMesh(Operator):
    ...
    def execute(self,c):
        ...
        mn=p.target_mesh
        bm=get_mesh_bm(bpy.data.objects[mn])
...
def _build_session_lines(mn):
    placed={d[0]:get_lm_pos(d[0],mn) for d in LANDMARK_DEFS if lm_placed(d[0],mn)}
    if not placed: return None,placed
    lines=["="*66,"  FARKAS FACIAL MEASUREMENTS (1994 standard)",
           f"  File: {bpy.data.filepath or 'unsaved'}",
           f"  Session / Mesh: {mn}","="*66,"",
    ...
    for a,b,lbl in MEASUREMENT_PAIRS:
        if a in placed and b in placed:
            dist=(mathutils.Vector(placed[a])-mathutils.Vector(placed[b])).length
            lines.append(f"  {lbl:<44}  {dist*1000:>8.2f} mm")
    ...
    for ka in keys:
        row=f"{ka:<10}"
        for kb in keys:
            row+=f"{'---':>10}" if ka==kb else                  f"{(mathutils.Vector(placed[ka])-mathutils.Vector(placed[kb])).length*1000:>10.2f}"
        lines.append(row)
    lines+=["","="*66]
    return lines,placed

def _build_comparison(sessions_data):
    if len(sessions_data)<2: return []
    names=[mn for mn,_ in sessions_data]; col_w=12
    lines=["","="*66,"  COMPARATIVE MEASUREMENTS","="*66]
    header=f"  {'Measurement':<40}"+"".join(f"{n[:col_w-1]:>{col_w}}" for n in names)
    lines.append(header); lines.append("-"*66)
    for a,b,lbl in MEASUREMENT_PAIRS:
        row=f"  {lbl:<40}"
        for mn,placed in sessions_data:
            if a in placed and b in placed:
                row+=f"{(mathutils.Vector(placed[a])-mathutils.Vector(placed[b])).length*1000:>{col_w}.1f}"
            else: row+=f"{'---':>{col_w}}"
        lines.append(row)
    pl0,pl1=sessions_data[0][1],sessions_data[1][1]
    lines+=["",f"  DELTA ({names[0]} vs {names[1]})","-"*50]
    for a,b,lbl in MEASUREMENT_PAIRS:
        if a in pl0 and b in pl0 and a in pl1 and b in pl1:
            d0=(mathutils.Vector(pl0[a])-mathutils.Vector(pl0[b])).length*1000
            d1=(mathutils.Vector(pl1[a])-mathutils.Vector(pl1[b])).length*1000
            lines.append(f"  {lbl:<40}  {d1-d0:>+8.2f} mm")
    lines+=["","="*66]
    return lines

class FBIO_OT_Export(Operator):
    bl_idname="fbio.export_measurements"; bl_label="Export TXT (sessione attiva)"
    def execute(self,c):
        p=c.scene.facial_biom; path=bpy.path.abspath(p.export_path)
        if not p.target_mesh: self.report({'ERROR'},"Nessuna mesh"); return {'CANCELLED'}
        lines,_=_build_session_lines(p.target_mesh)
        if not lines: self.report({'ERROR'},"Nessun landmark"); return {'CANCELLED'}
        try:
...
class FBIO_OT_ExportAll(Operator):
    bl_idname="fbio.export_all"; bl_label="Export TXT (tutte le sessioni)"
    def execute(self,c):
        p=c.scene.facial_biom; base_path=bpy.path.abspath(p.export_path)
        base,ext=os.path.splitext(base_path); all_path=base+"_ALL"+ext
        sessions=get_all_sessions()
        if not sessions: self.report({'ERROR'},"Nessuna sessione"); return {'CANCELLED'}
        all_lines=["="*66,"  IRIMeS — FARKAS MEASUREMENTS — ALL SESSIONS",
                   f"  File: {bpy.data.filepath or 'unsaved'}",
                   f"  Sessions: {len(sessions)}","="*66]
        sessions_data=[]
        for mn in sessions:
            lines,placed=_build_session_lines(mn)
            if lines: all_lines+=[""] + lines; sessions_data.append((mn,placed))
        if len(sessions_data)>=2:
            all_lines+=_build_comparison(sessions_data)
        try:
            os.makedirs(os.path.dirname(os.path.abspath(all_path)) or ".",exist_ok=True)
            open(all_path,'w',encoding='utf-8').write("\n".join(all_lines))
            self.report({'INFO'},f"{len(sessions_data)} sessioni -> {all_path}")
        except Exception as e: self.report({'ERROR'},str(e)); return {'CANCELLED'}
        return {'FINISHED'}

class FBIO_OT_DeleteSession(Operator):
    bl_idname="fbio.delete_session"; bl_label="Elimina sessione"
    mesh_name: StringProperty()
    def execute(self,c):
        delete_session(self.mesh_name); _tag()
        self.report({'INFO'},f"Sessione '{self.mesh_name}' eliminata"); return {'FINISHED'}

class FBIO_OT_ToggleSession(Operator):
    bl_idname="fbio.toggle_session"; bl_label="Mostra/Nascondi"
    mesh_name: StringProperty()
    def execute(self,c):
        cname=_col_name(self.mesh_name)
        if cname in bpy.data.collections:
            col=bpy.data.collections[cname]; col.hide_viewport=not col.hide_viewport; _tag()
        return {'FINISHED'}

class FBIO_OT_SwitchSession(Operator):
    bl_idname="fbio.switch_session"; bl_label="Passa a questa sessione"
    mesh_name: StringProperty()
    def execute(self,c):
        p=c.scene.facial_biom
        if self.mesh_name in bpy.data.objects: p.target_mesh=self.mesh_name
        _tag(); self.report({'INFO'},f"Sessione: {self.mesh_name}"); return {'FINISHED'}
...
class FBIO_PT_Main(Panel):
    ...
    def draw(self,c):
        ...
        b.prop(p,"export_path",text="")
        row_e=b.row(align=True)
        row_e.operator("fbio.export_measurements",icon='EXPORT',text="Sessione attiva")
        row_e.operator("fbio.export_all",icon='DOCUMENTS',text="Tutte")
...
class FBIO_PT_Sessions(Panel):
    bl_label="Sessioni Multiple"; bl_idname="FBIO_PT_Sessions"
    bl_space_type='VIEW_3D'; bl_region_type='UI'; bl_category="Biometry"
    bl_options={'DEFAULT_CLOSED'}
    def draw(self,c):
        L=self.layout; p=c.scene.facial_biom
        sessions=get_all_sessions()
        if not sessions:
            L.label(text="Nessuna sessione",icon='INFO')
            L.label(text="Piazza landmark su una mesh per creare una sessione",icon='BLANK1')
            return
        L.label(text=f"{len(sessions)} sessioni nel file",icon='OUTLINER_COLLECTION')
        for mn in sessions:
            col_obj=bpy.data.collections.get(_col_name(mn))
            n_placed=sum(1 for d in LANDMARK_DEFS if lm_placed(d[0],mn))
            is_active=(mn==p.target_mesh)
            box=L.box()
            row=box.row(align=True)
            if col_obj:
                icon_eye='HIDE_OFF' if not col_obj.hide_viewport else 'HIDE_ON'
                op=row.operator("fbio.toggle_session",text="",icon=icon_eye,emboss=False)
                op.mesh_name=mn
            op2=row.operator("fbio.switch_session",
                text=("▶  " if is_active else "     ")+f"{mn}  ({n_placed}/27)",
                icon='MESH_DATA', emboss=is_active)
            op2.mesh_name=mn
            op3=row.operator("fbio.delete_session",text="",icon='TRASH',emboss=False)
            op3.mesh_name=mn
        L.separator()
        row2=L.row(align=True)
        row2.operator("fbio.export_measurements",icon='EXPORT',text="Sessione attiva")
        row2.operator("fbio.export_all",icon='DOCUMENTS',text="Tutte")
...
# ─────────────────────────────────────────────────────────────────────────────
#  ADDON PREFERENCES  (visible in Edit → Preferences → Add-ons)
# ─────────────────────────────────────────────────────────────────────────────
class IRIMeSPreferences(bpy.types.AddonPreferences):
    bl_idname = __name__

    def draw(self, context):
        layout = self.layout
        row = layout.row()
        row.label(text="IRIMeS  —  Facial Biometric Landmarks", icon='MESH_DATA')
        layout.separator()
        box = layout.box()
        col = box.column(align=True)
        col.label(text="Author",  icon='USER')
        col.label(text="  Daniele Bursich (Ph.D.)")
        col.label(text="  University of Verona")
        col.separator()
        col.label(text="Project", icon='BOOKMARKS')
        col.label(text="  RESP — The Roman Emperor Seen from the Provinces")
        col.label(text="  ERC Consolidator Grant  ·  G.A.: 101002763")
        col.separator()
        col.label(text="Website", icon='URL')
        col.operator("wm.url_open", text="https://irimes.dcuci.univr.it/",
                     icon='WORLD').url = "https://irimes.dcuci.univr.it/"
        layout.separator()
        box2 = layout.box()
        col2 = box2.column(align=True)
        col2.label(text="Scientific Reference", icon='DOCUMENTS')
        col2.label(text="  Farkas, L.G. (1994). Anthropometry of the Head and Face,")
        col2.label(text="  2nd ed. Raven Press, New York.")
        layout.separator()
        layout.label(text="© 2024 Daniele Bursich — University of Verona. MIT License.",
                     icon='FUND')

classes=[
    IRIMeSPreferences, LandmarkGroupItem, FacialBiomProps,
    ...
    FBIO_OT_Export, FBIO_OT_ExportAll,
    FBIO_OT_DeleteSession, FBIO_OT_ToggleSession, FBIO_OT_SwitchSession,
    FBIO_UL_Groups,
    FBIO_PT_Main, FBIO_PT_Landmarks, FBIO_PT_Groups, FBIO_PT_Sessions, FBIO_PT_Measurements,
]
